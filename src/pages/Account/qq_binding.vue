			...
			registerInfo(){
				...
				if(_self.banding == 0){
					if( _self.isPhone(_self.msg1) && _self.isMsg(_self.msg2) ){
						const url = _self.$store.state.Yi + 'index/api/ThirdParty_qq';
						axios.post(url, qs.stringify({
							token: '',
							binding: _self.banding,
							openid: _self.$route.query.openid,
							unionid: _self.$route.query.unionid,
							user_data: JSON.stringify(_self.$store.state.usesData),
							phone: _self.msg1,
							smscode: _self.msg2,
							password: '',
							password_com: '',
					...
					if( this.isPhone(this.msg1) && this.isMsg(this.msg2) && this.psd1(this.msg3) && this.psd2(this.msg3,this.msg4) ){
						const url = this.$store.state.Yi + 'index/api/ThirdParty_qq';
						axios.post(url, qs.stringify({
							token: '',
							binding: this.banding,
							openid: this.$route.query.openid,
							unionid: this.$route.query.unionid,
							user_data: JSON.stringify(this.$store.state.usesData),
							phone: this.msg1,
							smscode: this.msg2,
							password: this.msg3,
							password_com: this.msg4,
							country_code: '86',
							login_type: this.$store.state.login_type,
						})).then(
							(res) => {
...
<style scoped="scoped">
	...
	.qq_binding ul .logo img{
		width: 4rem;
	}
	
	.qq_binding ul .tel,.qq_binding ul .psd{
		height: .9rem;
		margin: 0 .28rem;
		text-align: left;
		margin-bottom: .62rem;
	...
	.qq_binding ul .tel span,.qq_binding ul .psd span{
		position: absolute;
		left: 0;
		bottom: 5px;
		color: #e5000c;
	...
	.qq_binding ul .tel input,.qq_binding ul .psd input{
		position: absolute;
		left: 35px;
		bottom: 0;
		height: .6rem;
	...
	.qq_binding ul .logining{
		height: .9rem;
		margin: 0 .28rem;
	}
	
	.qq_binding ul .logining button{
		height: 100%;
		width: 100%;
	...
	.qq_binding ul .logining .downs{
		background-color: #e5000c;
	}
	
	.qq_binding ul .logining .down{
		background-color: #e5000c;
	}
