	...
	export default{
		...
		methods: {
			...
			getSiteInfo(){
				...
				if (!token) token = 0;
				const url = this.$store.state.Yi + 'index/My/xiugai';
				
				if(this.userName(this.msg9)&&this.site1(this.msg1)&&this.site2(this.msg2)&&this.isPhone(this.msg4)){
					// if(!this.msg5) this.msg5 = "";
					// if(!this.msg6) this.msg6 = "";
					// if(!this.msg7) this.msg7 = "";
			...
			//获取当前id地址
			getDizeditInfo(){
				const url = this.$store.state.Yi + 'index/My/dizedit';
				axios.post(url,qs.stringify({
					address_id: this.$route.query.address_id
				})).then(this.getDizeditSuc)
			},
			getDizeditSuc(res){
				//console.log(res)
				this.msg2 = res.data.data.bool.address;
				this.msg3 = res.data.data.bool.zipcode;
				this.msg9 = res.data.data.bool.consignee;
				this.msg4 = res.data.data.bool.mobile;
				this.msg5 = res.data.data.bool.dianhua.telfenji;
				this.msg6 = res.data.data.bool.dianhua.telhao;
				this.msg7 = res.data.data.bool.dianhua.telquhao;
				this.msg8 = res.data.data.bool.address_type;
				this.msg1 = res.data.data.bool.province + " " + res.data.data.bool.city + " " + res.data.data.bool.district;
				this.msg1_1 = res.data.data.bool.province;
				this.msg1_2 = res.data.data.bool.city;
				this.msg1_3 = res.data.data.bool.district;
			}
...
<style scoped="scoped">
	...
	.SettingSiteup ul>.tel{
		border-bottom: none;
	}
	
	.SettingSiteup ul li .title{
		flex: 1;
		height: 1rem;
		line-height: 1rem;
	}
	
	.SettingSiteup ul li>.tel{
		height: 3rem;
	}
	
	.SettingSiteup ul li .inp{
		flex: 3;
		height: 1rem;
		line-height: 1rem;
	}
	
	.SettingSiteup ul li .inp input{
		height: 92%;
		width: 100%;
	}
	
	.SettingSiteup ul li .telinp input{
		border-bottom: #ececec 1px solid;
	}
	
	.SettingSiteup ul li .inp input::-webkit-input-placeholder{
		color: #8e9bb1;
		font-size: .24rem;
	}
	
	.SettingSiteup ul .checked{
		height: 1rem;
		line-height: 1rem;
		text-align: center;
	}
	
	.SettingSiteup ul .checked p{
		/* width: 100%; */
		margin-left: 35%;
	}
	
	.SettingSiteup ul .checked .label{
		position: absolute;
		width: .3rem;
		height: .3rem;
	...
	.SettingSiteup ul .checked .label .active{
		position: absolute;
		width: .18rem;
		height: .18rem;
